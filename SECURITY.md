...
| Admin Safe (2/3) | policy inside caps, fee recipient, Valorem fee acceptance, deposit cap, role grants; on our own Clear, also that Clear's `feeTo` (fee switch, fee sweep, `setFeeTo`) | the bootstrap admin row, needing two of three signers instead of one key. No timelock on any of it |
...
| Clear `feeTo` of whichever clearinghouse the vault is constructed with (Overcall's EOA `0xdAe7…0782` on the default instance, an unfunded key with nonce 0; our admin on an instance from `script/DeployClear.s.sol`) | the 15 bps engine fee switch (`setFeesEnabled`), `setFeeTo` (which emits no event: nomination is visible only in storage), the URI generator, sweeping accumulated fee balances. Clear itself has no owner, no pause, no blocklist and no proxy | on an instance whose `feeTo` is NOT our admin: a week the vault refuses to arm or fill until governance accepts the fee (`ValoremFeeNotAccepted`); nothing on collateral. On the instance we deploy (the launch choice, `feeTo` = the admin) the switch and the acceptance sit with ONE key, so the admin rows above carry the fee: 15 bps of each fill's notional in NVDA, buyer-compensated through the fill floor. Upstream Valorem is dormant (last commit 2023-11), so there is no patch path, bounty or incident response behind either instance |
