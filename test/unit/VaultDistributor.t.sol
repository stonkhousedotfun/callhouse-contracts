...
///        protocol 10%     1.900000 USDG   (of the harvest, i.e. of the 19)
///        to holders      17.100000 USDG
contract VaultDistributorTest is BaseTest {
    ...
    /// @dev Two holders with different stakes, one filled week, exact split.
    function test_filledWeekSplitsProRata() public {
        ...
        // 17.100000 USDG over 30 shares, 1e27-scaled.
        assertEq(vault.accUsdgPerShare(), 570_000_000_000_000, "index credited");
        assertEq(vault.totalUsdgDistributed(), 17_100_000, "net of the 10% protocol fee");
        assertEq(vault.usdgDust(), 0, "30e18 shares divide 17.1 USDG exactly");

        assertEq(vault.claimableUsdg(alice), 11_400_000, "alice holds two thirds");
        assertEq(vault.claimableUsdg(bob), 5_700_000, "bob holds one third");
        assertEq(vault.claimableUsdg(alice) + vault.claimableUsdg(bob), 17_100_000, "the two claims are the whole net");
    ...
    function test_claimUsdgPaysAndZeroes() public {
        ...
        assertEq(owed, 17_100_000, "sole holder takes the whole net");
    ...
    function test_claimUsdgToSendsToThirdParty() public {
        ...
        assertEq(paid, 17_100_000, "amount");
        assertEq(usdg.balanceOf(carol), 17_100_000, "carol received it");
    ...
    function test_harvestDoesNotMoveTheSharePrice() public {
        ...
        assertEq(vault.claimableUsdg(alice), 11_400_000, "premium landed as a claim");
    ...
    function test_transferKeepsTheEarnedWeekWithTheSeller() public {
        ...
        _fullCycleOtm(10, _okUnitPrice());
        assertEq(vault.claimableUsdg(alice), 17_100_000, "week one is alice's");
        ...
        assertEq(vault.claimableUsdg(alice), 17_100_000, "alice keeps the week she sat through");
        assertEq(vault.claimableUsdg(bob), 0, "bob inherits shares, not accrued premium");

        // Week two: $3.00 x 10 => 28.500000 to the vault, 2.850000 fee, 25.650000 net.
        _newWeek();
        _fullCycleOtm(10, 3_000_000);

        assertEq(vault.claimableUsdg(bob), 25_650_000, "bob earns the week he actually held");
        assertEq(vault.claimableUsdg(alice), 17_100_000, "alice earns nothing after selling");
        ...
        vm.prank(bob);
        vault.claimUsdg();
        assertEq(usdg.balanceOf(alice), 17_100_000, "alice paid");
        assertEq(usdg.balanceOf(bob), 25_650_000, "bob paid");
    ...
    function test_lateDepositorEarnsNothingFromTheHarvestBeforeIt() public {
        ...
        assertEq(vault.claimableUsdg(alice), 17_100_000, "and took nothing from alice");

        // She does earn the next one, pro-rata on a 30e18 supply.
        _newWeek();
        _fullCycleOtm(10, _okUnitPrice());
        assertEq(vault.claimableUsdg(carol), 5_700_000, "one third of week two");
        assertEq(vault.claimableUsdg(alice), 28_500_000, "week one plus two thirds of week two");
    }
    ...
    /// @dev Fee is 10% of what the VAULT harvested, not of what the buyer paid: Overcall's 5%
    ///      never touches the vault, so charging on it would be charging on money we never had.
    function test_protocolFeeIsExactlyTenPercentAndLandsAtFeeSafe() public {
        _deposit(alice, 20e18);
        ...
        emit Vault.Harvest(1, 19_000_000, 1_900_000, 17_100_000);
        vm.prank(keeper);
        vault.rollClose();

        assertEq(usdg.balanceOf(feeSafe), 1_900_000, "fee landed at the fee safe");
        assertEq(usdg.balanceOf(feeSafe) * 10, toVault, "exactly a tenth of the harvest");
        assertEq(vault.totalUsdgDistributed(), 17_100_000, "the other nine tenths went to holders");
    }
    ...
    /// @dev "10% of USDG harvested (filled weeks only)". A week nobody bought must cost the
    ...
    function test_unfilledWeekIsFreeAndClosesCleanly() public {
        ...
        _newWeek();
        _fullCycleOtm(10, _okUnitPrice());
        assertEq(vault.claimableUsdg(alice), 11_400_000, "the quiet week cost nobody anything");
    }
    ...
    function test_twoFilledWeeksAccumulate() public {
        ...
        _fullCycleOtm(10, _okUnitPrice());
        assertEq(vault.accUsdgPerShare(), 855_000_000_000_000, "week one index");
        assertEq(vault.claimableUsdg(alice), 17_100_000, "week one");

        // Week two: $3.00 x 5 => 14.250000 to the vault, 1.425000 fee, 12.825000 net.
        _newWeek();
        _fullCycleOtm(5, 3_000_000);

        assertEq(vault.accUsdgPerShare(), 1_496_250_000_000_000, "index accumulated, not replaced");
        assertEq(vault.claimableUsdg(alice), 29_925_000, "both weeks");
        assertEq(vault.totalUsdgDistributed(), 29_925_000, "lifetime distributed");
        assertEq(usdg.balanceOf(feeSafe), 3_325_000, "both protocol fees");
        assertEq(vault.usdgDust(), 0, "no dust at these sizes");

        vm.prank(alice);
        assertEq(vault.claimUsdg(), 29_925_000, "one claim collects both weeks");
        assertEq(usdg.balanceOf(address(vault)), 0, "nothing stranded");
    }
    ...
    ///      unit of `accUsdgPerShare` here is worth 10.000000 USDG in total, so a 4.275000
    ///      harvest rounds to nothing. That remainder must be CARRIED, not dropped — dropped
    ...
    function test_dustIsCarriedAndCreditedLater() public {
        ...
        // Week one: $1.00 x 5 => 4.750000 to the vault, 0.475000 fee, 4.275000 net.
        ...
        assertEq(vault.usdgDust(), 4_275_000, "the whole net is carried as dust");
        assertEq(usdg.balanceOf(feeSafe), 475_000, "the fee was still taken on a real harvest");
        assertEq(usdg.balanceOf(address(vault)), 4_275_000, "the money is in the vault, just unindexed");

        // Week two: $2.00 x 50 => 95.000000 to the vault, 9.500000 fee, 85.500000 net.
        // The pot is 85.500000 + the 4.275000 carried = 89.775000, which indexes to 8 units
        // (80.000000) and carries 9.775000 forward again.
        _newWeek();
        _fullCycleOtm(50, _okUnitPrice());

        assertEq(vault.accUsdgPerShare(), 8, "eight index units");
        assertEq(vault.totalUsdgDistributed(), 80_000_000, "credited");
        assertEq(vault.claimableUsdg(alice), 80_000_000, "including the week that alone rounded away");
        assertEq(vault.usdgDust(), 9_775_000, "the new remainder is carried in turn");
        assertEq(
            vault.totalUsdgDistributed() + vault.usdgDust(),
            4_275_000 + 85_500_000,
        ...
        vm.prank(alice);
        assertEq(vault.claimUsdg(), 80_000_000, "claimable pays out");
        assertEq(usdg.balanceOf(address(vault)), 9_775_000, "exactly the carried dust remains");
        assertEq(vault.usdgDust(), 9_775_000, "and it is still accounted for");
    }
    ...
    function test_unallocatedUsdgIsCarriedWhenSupplyIsZero() public {
        ...
        // One contract at $2.00 => 1.900000 to the vault, 0.190000 fee, 1.710000 net.
        _fullCycleOtm(1, _okUnitPrice());

        assertEq(vault.usdgUnallocated(), 1_710_000, "parked, with nobody to credit");
        assertEq(vault.accUsdgPerShare(), 0, "index cannot move with no supply");
        assertEq(vault.totalUsdgDistributed(), 0, "nothing distributed");
        assertEq(vault.usdgDust(), 0, "not dust - a different bucket");
        assertEq(usdg.balanceOf(feeSafe), 190_000, "the fee was taken");
        assertEq(usdg.balanceOf(address(vault)), 1_710_000, "held, not lost");
        ...
        // Week two: $2.00 x 10 => 19.000000 to the vault, 1.900000 fee, 17.100000 net.
        // The orphaned 1.710000 rides along and is NOT charged a fee a second time.
        _newWeek();
        _fullCycleOtm(10, _okUnitPrice());

        assertEq(usdg.balanceOf(feeSafe), 190_000 + 1_900_000, "fee charged once per harvest only");
        assertEq(vault.usdgUnallocated(), 0, "the parked USDG was spent into the index");
        assertEq(vault.totalUsdgDistributed(), 18_810_000, "17.100000 + the carried 1.710000");
        assertEq(vault.claimableUsdg(alice), 18_810_000, "the first holder picks up the orphaned week");
        assertEq(vault.usdgDust(), 0, "10 shares divide it exactly");

        _assertUsdgInvariant("unallocated carry");

        vm.prank(alice);
        assertEq(vault.claimUsdg(), 18_810_000, "paid in full");
    ...
    function test_usdgInvariantHoldsThroughQueueAndClaims() public {
        ...
        assertEq(vault.claimableUsdg(alice), 11_400_000, "week one, alice");
        assertEq(vault.claimableUsdg(bob), 5_700_000, "week one, bob");
        ...
        assertEq(vault.claimableUsdg(bob), 5_700_000, "queueing settled bob first, losing him nothing");
        ...
        // Week two's 17.100000 net was split across the full 30e18 supply, escrow included.
        assertEq(vault.claimableUsdg(alice), 22_800_000, "alice: both weeks at two thirds");
        assertEq(vault.claimableUsdg(bob), 8_550_000, "bob: week one plus his unqueued half");
        assertEq(vault.usdgReservedForQueue(), 2_850_000, "the escrow's earnings follow the redeemer");
        assertEq(vault.claimableUsdg(address(vault)), 0, "escrow swept, not left to the stayers");
        assertEq(usdg.balanceOf(address(vault)), 34_200_000, "two weeks of net premium");
        assertEq(_owedUsdg(), 34_200_000, "and every cent of it is spoken for");
        ...
        assertEq(usdgOut, 2_850_000, "plus the escrow's week");
        ...
        assertEq(usdg.balanceOf(bob), 8_550_000 + 2_850_000, "bob got his third of both weeks");
        assertEq(usdg.balanceOf(alice), 22_800_000, "alice got her two thirds of both weeks");
        assertEq(usdg.balanceOf(address(vault)), 0, "zero dust left in the vault");
    ...
    ///          10% of that (1.900000) and 17.100000 indexes over a 30e18 supply.
    ///          alice 20/30 -> 11.400000, bob 10/30 -> 5.700000.
    ///        week two is the same 17.100000 net, but alice moves 10e18 to bob AFTER the fill
    ///          and BEFORE the close, so the index applies to the post-transfer balances:
    ///          alice 10/30 -> 5.700000, bob 20/30 -> 11.400000.
    ///        both therefore end on 11.400000 + 5.700000 == 17.100000, exactly half the
    ///        34.200000 distributed over the two weeks, despite never having held equal stakes
    ///        at the same time.
    function test_midCycleTransferHandsTheUnindexedWeekToTheBuyer() public {
        ...
        _fullCycleOtm(10, _okUnitPrice());
        assertEq(vault.claimableUsdg(alice), 11_400_000, "week one, two thirds");
        assertEq(vault.claimableUsdg(bob), 5_700_000, "week one, one third");
        ...
        assertEq(usdg.balanceOf(address(vault)), 17_100_000 + 19_000_000, "week two's premium has landed");
        ...
        assertEq(vault.claimableUsdg(alice), 11_400_000, "alice keeps week one and not a unit more");
        assertEq(vault.claimableUsdg(bob), 5_700_000, "bob has not been credited week two either - yet");
        ...
        assertEq(vault.totalUsdgDistributed(), 34_200_000, "two weeks of net premium indexed");
        assertEq(vault.claimableUsdg(alice), 17_100_000, "11.400000 earned, then only 5.700000 of week two");
        assertEq(vault.claimableUsdg(bob), 17_100_000, "5.700000 earned, then 11.400000 of the week he bought into");
        assertEq(
        ...
        vm.prank(alice);
        assertEq(vault.claimUsdg(), 17_100_000, "alice paid in full");
        vm.prank(bob);
        assertEq(vault.claimUsdg(), 17_100_000, "bob paid in full");
    ...
    ///        The protocol fee is 10% of the 4_691_356 harvested, floored: 469_135
    ///          (469_135.6 truncated), leaving 4_222_221 for holders. 469_135 * 10 is
    ///          4_691_350, six units short of the gross - and those six units go to the
    ///          holders, not the fee safe. Rounding must favour the depositors.
    function test_partialFillHarvestsOnlyWhatWasSold() public {
        ...
        vm.expectEmit(true, false, false, true, address(vault));
        emit Vault.Harvest(1, 4_691_356, 469_135, 4_222_221);
        vm.prank(keeper);
        vault.rollClose();

        assertEq(usdg.balanceOf(feeSafe), 469_135, "fee is 10% of the partial harvest, floored");
        assertLt(usdg.balanceOf(feeSafe) * 10, 4_691_356, "the flooring shortfall stays with the holders");
        assertEq(vault.totalUsdgDistributed(), 4_222_221, "and every remaining unit was indexed");
        assertEq(vault.claimableUsdg(alice), 4_222_221, "sole holder takes all of it");
    ...
    function testFuzz_claimsNeverExceedWhatWasDistributed(
        ...
        uint256 expectedFee = (toVault * 1_000) / 10_000;
        uint256 expectedNet = toVault - expectedFee;

        _fullCycleOtm(n, unitPrice);

        assertEq(usdg.balanceOf(feeSafe), expectedFee, "protocol fee is 10% of the harvest");
        ...
        public
    {
        ...
        (uint256 toVault,,) = _splitPremium(unitPrice, n);
        uint256 pot = toVault - (toVault * 1_000) / 10_000;
