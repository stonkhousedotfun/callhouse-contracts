# Review scope — v1 (historical)

> **This document scopes the v1 Seaport/Valorem covered-call Vault, not v2 and not
> INTERFACE_VERSION 8.** It was `docs/AUDIT-SCOPE.md` until 2026-09-20, when `C8-11` gave that name
> to the v8 audit packet and moved this text here unchanged. Nothing in it is in scope for the v8
> audit: go to [AUDIT-SCOPE.md](AUDIT-SCOPE.md). The v1 contracts are in run-off
> ([V1-RUNOFF.md](V1-RUNOFF.md)); the sentence below about a pending audit is the 2026-09-15 state and
> is superseded by owner decision V3-D33 (see [SECURITY.md](../SECURITY.md), "v2 → Status"). The rest
> is kept verbatim, with its dates and its line numbers as they stood.
...
(`grant`, then `renounce` refused until the Safe has executed a transaction after the grant); it
never moves Clear's `feeTo`. `Verify.s.sol` is read-only and reverts if any check fails. On our own
Clear it also requires `EXPECTED_CLEAR_FEE_TO`, pins the vendored runtime, and checks `feeTo` and
that no `setFeeTo` nomination is pending (path A: 67 / 73 / 76; path B Overcall: 71; `docs/DEPLOY.md`): vault and both libraries byte for byte against `out/` with link
